...
class CNNTransformer(nn.Module):
    def __init__(self, in_channels, d_model, nhead, num_layers):
        ...
        self.decoder_ = CNNEncoder(d_model, d_model)
    ...
    def forward(self, x):
        ...
        # x = x.permute((3,0,1,2)).flatten(2) # for wide direction transformer, -> (W,N,C*H)=(W, N, d_model*H), wrong , this will make the params of multi-head attn be wrong.
        x = x.permute((3, 1, 0,2)).flatten(2).permute((0,2,1)) # -> (W, C, N*H)=(W,d_model, N*H) ->(W,N*H,d_model)
        x = self.transformer_encoder(x) # wide direction
        ...
        # x = x.view((W,N,self.d_model, H)).permute((1,2,3,0)) # (W,N,d_model*H)
        x = x.permute((0,2,1)).view((W,self.d_model, N,H)).permute((2,1,3,0))
        x = self.decoder_(x)
        x = self.decoder(x)
